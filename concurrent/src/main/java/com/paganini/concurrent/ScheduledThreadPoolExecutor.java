...
public class ScheduledThreadPoolExecutor extends ThreadPoolExecutor
    ...
    */
    ...
    /**
     * 在线程池关闭的状态下是否继续执行周期性任务（true，继续执行正在执行的调度任务）
    * False if should cancel/suppress periodic tasks on shutdown.
    */
    private volatile boolean        continueExistingPeriodicTasksAfterShutdown;

    /**
     * 在线程池关闭的状态下是否继续执行延时任务（true，继续执行正在执行的延时任务）
    ...
    */
    ...
    private class ScheduledFutureTask<V> extends FutureTask<V>
        ...
        /**
         * 周期性任务的间隔时间，正数表示；负数表示fixed-delay执行；0表示非重复调度任务
    ...
    */
    ...
    /**
     * 到达延迟时间后执行周期性调用的任务
     * delay是前一个任务执行完成后，和下一个任务开始时间的间隔
    * @throws RejectedExecutionException {@inheritDoc}
    * @throws NullPointerException       {@inheritDoc}
    * @throws IllegalArgumentException   {@inheritDoc}
    */
